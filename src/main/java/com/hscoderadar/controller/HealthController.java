 ...
 *
 * <p>이 컨트롤러는 서버 상태 확인과 기본 홈 페이지 응답을 담당함. 모든 응답은 자동으로 {@link
 * com.hscoderadar.common.response.ApiResponse}로 래핑됨.
 *
...
public class HealthController {
   ...
   * 홈 페이지 기본 응답을 반환
   *
   * <p>애플리케이션의 기본 홈 페이지 메시지를 반환하며, 클라이언트가 서버에 정상적으로 연결되었는지 확인할 수 있음.
   *
   * @return 홈 페이지 환영 메시지
   *     <h3>API 호출 예시:</h3>
   *     <pre>
   ...
   *
   * <p>서버가 정상적으로 작동 중인지 확인할 수 있는 상태 체크 API. 로드 밸런서나 모니터링 시스템에서 사용할 수 있음.
   *
   * @return 서버 상태 메시지
   *     <h3>API 호출 예시:</h3>
   *     <pre>

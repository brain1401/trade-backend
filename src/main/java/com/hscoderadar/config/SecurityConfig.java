 ...
 *
 * <p>v6.1 변경된 JWT 토큰 정책을 적용하는 보안 설정. Public API와 Private API를 구분하여 차별화된 보안 정책을 적용:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p><strong>v6.1 JWT 토큰 정책:</strong>
 *
 * <ul>
...
public class SecurityConfig {
   ...
   *
   * <p>AuthenticationManager는 사용자 인증 처리의 핵심 컴포넌트로, 로그인 시 사용자 자격증명(이메일/비밀번호)을 검증하는 역할을 담당함.
   *
   ...
   *
   * <p>BCryptPasswordEncoder는 단방향 해시 함수를 사용하여 비밀번호를 안전하게 암호화함. Salt를 자동으로 생성하여 동일한 비밀번호라도 서로 다른
   * 해시값을 생성함.
   *
   ...
   *
   * <p>개발 단계에서는 복잡한 CORS 정책보다는 간단하고 명확한 설정을 사용. 모든 API에 대해 localhost:3000에서의 접근을 허용하며, HttpOnly 쿠키를
   * 지원.
   *
   ...
   *
   * <p>Public API와 Private API를 구분하여 차별화된 보안 정책을 적용하며, HttpOnly 쿠키 기반 JWT 인증과 OAuth2 로그인을 통합 지원함.
   *
   * <h3>URL별 접근 정책:</h3>
   *
   * <ul>

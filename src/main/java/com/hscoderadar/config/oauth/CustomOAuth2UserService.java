 ...
 *
 * <p>Spring Security OAuth2 클라이언트의 {@link DefaultOAuth2UserService}를 상속받아 Google, Naver, Kakao 등의
 * OAuth2 제공업체별 사용자 정보를 처리하고 데이터베이스와 연동하여 사용자 등록/업데이트 및 SNS 계정 연동 관리 수행
 *
 * <h3>v4.2 주요 개선사항:</h3>
 *
 * <ul>
 *   <li>SNS 계정 연동 테이블(sns_accounts) 활용
 *   <li>Spring Session 기반 세션 관리 호환성
 *   <li>프로필 이미지 처리 개선
 *   <li>계정 통합 관리 로직 강화
 * </ul>
 *
 ...
 *
 * <ol>
 *   <li>OAuth2 제공업체에서 사용자 정보 수신
 *   <li>제공업체별 응답 구조에 따른 정보 파싱
 *   <li>이메일을 기준으로 기존 사용자 조회
 *   <li>SNS 계정 연동 정보 확인 및 업데이트
 *   <li>신규 사용자인 경우 DB에 등록, 기존 사용자인 경우 정보 업데이트
 *   <li>Spring Security용 PrincipalDetails 객체 생성 및 반환
 * </ol>
 *
...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
  ...
  public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
    ...
    OAuth2UserInfo oAuth2UserInfo =
        createOAuth2UserInfo(registrationId, oAuth2User.getAttributes());
    log.debug(
        "OAuth2 사용자 정보 파싱 완료: email={}, name={}, provider={}",
  ...
  private User processUser(OAuth2UserInfo oAuth2UserInfo, String registrationId) {
    ...
    if (existingUser.isPresent()) {
      ...
      log.info(
          "기존 OAuth2 사용자 로그인: userId={}, email={}, provider={}",
          user.getId(),
          user.getEmail(),
          registrationId);
   ...
   * @param registrationId OAuth2 제공업체 식별자
   */
  private void processSnsAccountForExistingUser(
      User user, OAuth2UserInfo oAuth2UserInfo, String registrationId) {
    SnsProvider provider = mapToProvider(registrationId);
    String providerId = oAuth2UserInfo.getProviderId();

    // 해당 제공업체의 SNS 계정 연동 정보 확인
    Optional<SnsAccount> existingSnsAccount =
        snsAccountRepository.findByUserAndProvider(user, provider);
    ...
    if (existingSnsAccount.isPresent()) {
      ...
      if (needsUpdate) {
        ...
        log.debug("기존 SNS 계정 연동 정보 업데이트: userId={}, provider={}", user.getId(), provider);
      }
    } else {
      // 새로운 SNS 계정 연동 추가
      SnsAccount newSnsAccount =
          SnsAccount.builder()
              .user(user)
              .provider(provider)
      ...
      log.info("새로운 SNS 계정 연동 추가: userId={}, provider={}", user.getId(), provider);
    }
  }
  ...
  private User createNewUserWithSnsAccount(OAuth2UserInfo oAuth2UserInfo, String registrationId) {
    ...
    log.info(
        "신규 OAuth2 사용자 등록 완료: userId={}, email={}, provider={}, profileImage={}",
        savedUser.getId(),
        savedUser.getEmail(),
        registrationId,
  ...
  private void updateProfileImageIfNeeded(User user, OAuth2UserInfo oAuth2UserInfo) {
    ...
    if (newProfileImage != null && !newProfileImage.equals(currentProfileImage)) {
      if (isValidProfileImageUrl(newProfileImage)) {
        ...
        log.debug(
            "OAuth2 사용자 프로필 이미지 업데이트: userId={}, profileImage={}", user.getId(), newProfileImage);
      } else {
        log.warn("유효하지 않은 프로필 이미지 URL: userId={}, url={}", user.getId(), newProfileImage);
      }
    }
  }

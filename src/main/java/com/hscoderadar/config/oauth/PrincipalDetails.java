 ...
 * 
 * <p>
 * 일반 로그인과 OAuth2 로그인을 모두 지원하는 어댑터 클래스로,
 * Spring Security의 {@link UserDetails}와 {@link OAuth2User} 인터페이스를
 * 동시에 구현하여 두 가지 인증 방식을 통일적으로 처리
 * 
 * <h3>지원하는 인증 방식:</h3>
 * <ul>
 * <li><strong>일반 로그인:</strong> 이메일/비밀번호 기반 인증</li>
 * <li><strong>OAuth2 로그인:</strong> Google, Naver, Kakao 소셜 로그인</li>
 * </ul>
 * 
 * <h3>Spring Security 통합:</h3>
 * <ul>
 * <li>{@link UserDetails} 구현으로 일반 인증 지원</li>
 * <li>{@link OAuth2User} 구현으로 OAuth2 인증 지원</li>
 * <li>SecurityContext에서 일관된 사용자 정보 접근</li>
 * </ul>
 * 
 * <h3>사용 예시:</h3>
 * 
...
public class PrincipalDetails implements UserDetails, OAuth2User {
     ...
     * 
     * <p>
     * 이메일/비밀번호 기반 인증 시 사용되며,
     * OAuth2 속성은 null로 설정됨
     * 
     ...
     * 
     * <p>
     * 소셜 로그인 성공 시 {@link CustomOAuth2UserService}에서 호출되며,
     * OAuth2 제공업체의 원본 사용자 속성 정보를 함께 저장
     * 
     * @param user       인증된 사용자 엔티티
     ...
     * 
     * <p>
     * 현재는 모든 사용자에게 ROLE_USER 권한을 부여하며,
     * 향후 역할 기반 권한 관리로 확장 가능
     * 
     * @return 사용자 권한 컬렉션
     */
    @Override
     ...
     * 
     * <p>
     * OAuth 사용자의 경우 비밀번호 해시가 null이므로,
     * 일반 로그인 시도 시 인증이 실패하도록 빈 문자열 반환
     * 

 ...
 *
 * <p>Google OAuth2 로그인 시 Google에서 제공하는 사용자 정보를 애플리케이션 내부에서 사용할 수 있는 형태로 변환하는 어댑터 클래스
 *
 ...
 *
 * <ul>
 *   <li>Google의 sub 필드를 providerId로 사용하여 고유성 보장
 *   <li>이메일은 Google에서 인증된 상태로 제공
 *   <li>null 또는 예상치 못한 데이터에 대한 안전한 처리
 * </ul>
 *
...
public class GoogleUserInfo implements OAuth2UserInfo {
   ...
   *
   * <p>Google에서 제공받은 사용자 속성 정보를 저장하고, null인 경우 빈 Map으로 초기화하여 NullPointerException 방지
   *
   ...
   *
   * <p>Google OAuth2에서 제공하는 'sub' 필드는 Google 내에서 사용자를 고유하게 식별하는 불변의 식별자
   *
   * <h3>특징:</h3>
   *
   * <ul>
   *   <li>Google 계정마다 고유한 값
   *   <li>변경되지 않는 영구적 식별자
   *   <li>이메일 변경과 무관하게 동일 사용자 식별 가능
   * </ul>
   *
   ...
   *
   * <p>Google OAuth2에서는 사용자 이메일이 기본적으로 제공되며, Google에서 이미 인증된 상태의 이메일 주소
   *
   * <h3>특징:</h3>
   *
   * <ul>
   *   <li>Google에서 인증된 유효한 이메일 주소
   *   <li>기본 로그인 식별자로 사용
   *   <li>이메일 인증 절차 불필요
   * </ul>
   *
   ...
   *
   * <p>Google OAuth2에서 제공하는 'name' 필드는 사용자가 Google 계정에 설정한 표시 이름으로, 실명이거나 사용자가 선택한 닉네임일 수 있음
   *
   * <h3>특징:</h3>
   *
   * <ul>
   *   <li>사용자가 Google 계정에 설정한 표시 이름
   *   <li>실명 또는 닉네임 형태
   *   <li>사용자에 의해 변경 가능
   * </ul>
   *
   ...
   *
   * <p>Google OAuth2에서 제공하는 'picture' 필드는 사용자가 Google 계정에 설정한 프로필 이미지 URL로, 일반적으로 Google 서버에 호스팅되는
   * 이미지
   *
   * <h3>특징:</h3>
   *
   * <ul>
   *   <li>Google 서버에서 제공되는 프로필 이미지 URL
   *   <li>사용자가 프로필 이미지를 설정하지 않은 경우 null 반환
   *   <li>이미지 크기는 Google에서 자동 조정
   *   <li>HTTPS 프로토콜로 제공
   * </ul>
   *

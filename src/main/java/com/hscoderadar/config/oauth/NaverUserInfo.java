 ...
 * 
 * <p>
 * Naver OAuth2 로그인 시 네이버에서 제공하는 사용자 정보를
 ...
 * <h3>네이버 특이사항:</h3>
 * <ul>
 * <li>실제 사용자 정보는 'response' 키 안에 중첩되어 제공</li>
 * <li>외부 래퍼에는 API 호출 결과 코드와 메시지 포함</li>
 * <li>사용자 동의에 따라 일부 정보는 제공되지 않을 수 있음</li>
 * <li>고유 ID는 문자열 형태의 숫자로 제공</li>
 * </ul>
 * 
 * <h3>보안 고려사항:</h3>
 * <ul>
 * <li>네이버 회원 고유 ID를 providerId로 사용하여 고유성 보장</li>
 * <li>이메일은 네이버에서 인증된 상태로 제공</li>
 * <li>response 구조에서 안전한 데이터 추출</li>
 * </ul>
 * 
...
public class NaverUserInfo implements OAuth2UserInfo {
     ...
     * 
     * <p>
     * 네이버 OAuth2 응답의 특수한 구조를 처리하여 실제 사용자 정보만 추출함
     * 네이버는 다른 OAuth2 제공자와 달리 'response' 키 안에 실제 사용자 정보를 중첩하여 제공
     * 
     * <h3>처리 과정:</h3>
     * <ol>
     * <li>전체 응답 데이터에서 'response' 키 추출</li>
     * <li>OAuth2MapUtils를 사용한 안전한 타입 변환</li>
     * <li>null 체크 및 예외 상황 처리</li>
     * </ol>
     * 
     * <h3>네이버 응답 구조 특징:</h3>
     * <ul>
     * <li>외부 래퍼: resultcode, message (API 호출 결과)</li>
     * <li>실제 데이터: response 객체 내부</li>
     * <li>다른 OAuth2 제공자와 구조가 상이함</li>
     * </ul>
     * 
     ...
     * 
     * <p>
     * 'response' 키에서 추출된 실제 사용자 정보만 포함됨
     * 
     ...
     * 
     * <p>
     * 네이버 OAuth2에서 제공하는 'id' 필드는 네이버 회원의 고유 식별자로,
     * 네이버 내에서 사용자를 영구적으로 식별하는 문자열 형태의 숫자
     * 
     * <h3>특징:</h3>
     * <ul>
     * <li>네이버 계정마다 고유한 문자열 숫자</li>
     * <li>변경되지 않는 영구적 식별자</li>
     * <li>이메일이나 닉네임 변경과 무관하게 동일 사용자 식별 가능</li>
     * <li>다른 사용자와 절대 중복되지 않음</li>
     * </ul>
     * 
     ...
     * 
     * <p>
     * 네이버 OAuth2에서는 사용자가 동의한 경우에만 이메일 정보가 제공됨
     * 네이버에서 이미 인증된 상태의 이메일 주소이므로 별도 인증 불필요
     * 
     * <h3>특징:</h3>
     * <ul>
     * <li>네이버에서 인증된 유효한 이메일 주소</li>
     * <li>사용자 동의 시에만 제공 (필수 항목 아님)</li>
     ...
     * 
     * <p>
     * 네이버 OAuth2에서 제공하는 'name' 필드는 사용자가 네이버 계정에
     * 등록한 실명 정보로, 대부분 실명으로 제공
     * 
     * <h3>특징:</h3>
     * <ul>
     * <li>사용자가 네이버 계정에 등록한 실명</li>
     * <li>네이버 실명 인증 정책에 따라 검증된 이름</li>
     * <li>사용자 동의 시에만 제공</li>
     * <li>닉네임과는 별도의 필드</li>
     * </ul>
     * 
     ...
     * 
     * <p>
     * 네이버 OAuth2에서 제공하는 'profile_image' 필드는 사용자가 네이버 계정에
     * 설정한 프로필 이미지 URL로, 네이버 서버에 호스팅되는 이미지
     * 
     * <h3>특징:</h3>
     * <ul>
     * <li>네이버 서버에서 제공되는 프로필 이미지 URL</li>

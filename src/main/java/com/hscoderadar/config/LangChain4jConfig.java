 ...
 * 
 * ChatGPT 스타일 통합 채팅을 위한 LangChain4j 컴포넌트들을 Spring Bean으로 구성
 * - Claude AI 모델 설정 (내장 웹검색 기능 활용)
 * - 채팅 메모리 관리
 * - AI 서비스 체이닝
 */
@Configuration
...
public class LangChain4jConfig {
   ...
   * 
   * LangChain4j 1.1.0부터 ChatLanguageModel이 ChatModel로 변경됨
   * Anthropic Claude 모델을 수동으로 구성
   */
   ...
   * 
   * LangChain4j의 @UserMessage, @SystemMessage 어노테이션을 사용하여
   * Claude AI와의 구조화된 대화를 지원하는 인터페이스
   * 
   * 🌟 Claude의 내장 웹검색 기능을 자동으로 활용합니다!
   */
  public interface TradeAnalysisAI {
    ...
    @dev.langchain4j.service.SystemMessage("""
        당신은 무역 전문가입니다. 사용자의 질문을 분석하여 다음 중 하나의 의도로 분류해주세요:
    ...
    @dev.langchain4j.service.SystemMessage("""
        **[IMPORTANT: 웹 검색 필수 수행]**
        당신은 HS Code 분류 전문가입니다. 다음 단계를 반드시 따라 답변하세요:
     ...
     */
    @dev.langchain4j.service.SystemMessage("""
        당신은 화물 추적 전문가입니다. 화물 추적 데이터를 분석하여 사용자가 이해하기 쉽게 설명해주세요:
        ...
        필요하다면 최신 통관 정보를 웹에서 검색하여 더 정확한 정보를 제공해주세요.
        기술적인 용어는 일반인이 이해할 수 있도록 쉽게 설명해주세요.
        """)
    ...
    @dev.langchain4j.service.SystemMessage("""
        **[IMPORTANT: 실시간 웹검색 필수]**
        당신은 무역 컨설턴트입니다. 다음 단계를 반드시 수행하세요:
     ...
     */
    @dev.langchain4j.service.SystemMessage("""
        사용자에게 현재 AI가 수행하고 있는 작업을 투명하게 설명해주세요.
        한 문장으로 간결하게, 현재 진행 상황을 알려주세요.
   ...
   * 
   * LangChain4j AiServices를 사용하여 TradeAnalysisAI 인터페이스의
   * 구현체를 자동 생성하고 Claude AI 모델과 연결
   */
  @Bean

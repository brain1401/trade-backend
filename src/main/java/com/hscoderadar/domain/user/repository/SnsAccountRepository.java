 ...
 * 
 * OAuth 소셜 로그인 및 계정 연동 관리를 위한
 * SNS 계정 정보 조회 및 관리 기능 제공
 */
@Repository
public interface SnsAccountRepository extends JpaRepository<SnsAccount, Long> {
   ...
   * 여러 제공업체에 연동된 사용자 조회
   */
  @Query("SELECT sa.user FROM SnsAccount sa " +
      "GROUP BY sa.user " +
      "HAVING COUNT(sa.provider) > 1")
  List<User> findUsersWithMultipleProviders();

  /**
   * 특정 제공업체만 연동된 사용자 조회
   */
  @Query("SELECT DISTINCT sa.user FROM SnsAccount sa " +
  ...
  List<User> findUsersWithAllProviders(@Param("providers") List<SnsProvider> providers,
      @Param("providerCount") long providerCount);
}
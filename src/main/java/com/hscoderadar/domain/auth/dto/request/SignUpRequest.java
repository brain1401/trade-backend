 ...
 *
 * <p>📊 v6.1 보안 정책에 따른 회원가입 요청 정보: - 최소 필수 정보만 수집 (이메일, 비밀번호, 이름) - 강화된 비밀번호 정책 적용 - 응답에는 최소 사용자
 * 정보만 제공
 *
 * <p>🛡️ v6.1 보안 특징: - BCrypt 암호화 적용 - 이메일 중복 검증 - 사용자 열거 공격 방지
 *
...
public record SignUpRequest(
     ...
     * 회원가입할 사용자의 이메일 주소
     *
     * <p>이메일은 사용자의 고유 식별자로 사용되며, 시스템 내에서 중복될 수 없음. 로그인 시에도 이 이메일을 사용하게 됨.
     *
     * <h3>제약사항:</h3>
     *
     * <ul>
     ...
     * 사용자의 로그인 비밀번호
     *
     * <p>클라이언트에서 평문으로 전송되지만, 서버에서 BCrypt 알고리즘을 사용하여 안전하게 해시화되어 데이터베이스에 저장됨.
     *
     * <h3>보안 권장사항:</h3>
     *
     * <ul>
     ...
     * 사용자의 실명 또는 표시 이름
     *
     * <p>시스템 내에서 사용자를 식별하고 표시하는 데 사용되는 이름.
     *
     * <h3>제약사항:</h3>
     *
     * <ul>
   ...
   *
   * <p>이 메서드는 회원가입 요청 데이터를 실제 데이터베이스에 저장할 수 있는 User 엔티티 객체로 변환함.
   *
   * <h3>v4.2 변환 과정:</h3>
   *
   * <ol>
